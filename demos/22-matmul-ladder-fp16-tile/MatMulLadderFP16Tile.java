/*
 * TornadoVM CUDA demo — the FP16 matrix-multiply ladder, with a CUDA Tile rung.
 *
 * Demo 18 is this ladder without rung 4. Rung 4 is the point of this demo: the
 * same GEMM written against TileContext, compiled through NVIDIA CUDA Tile
 * rather than the SIMT path, sitting between the hand-written tensor-core rung
 * and the vendor libraries so all three are measured the same way.
 *
 *   1. naive @Parallel            one thread per output element
 *   2. KernelContext tiled        shared-memory tiles, FP32 accumulate
 *   3. KernelContext MMA          ctx.mma -> mma.sync.aligned.m16n8k16, by hand
 *   4. TileContext                tc.mma -> ct::mma, tile compiler picks the HMMA
 *   5. CUTLASS hgemm              library task, tensor cores
 *   6. cuBLAS GemmEx FP16         FP16 in, FP16 out, FP32 accumulate
 *   7. cuBLAS GemmEx FP16->FP32   FP16 in, FP32 out, the inference configuration
 *
 * Rungs 3 and 4 reach the same hardware from the same language and are the
 * comparison worth reading: rung 3 packs fragments, stages shared memory and
 * names a fixed m16n8k16 shape; rung 4 says mma and nothing else.
 *
 ...
 *
 * Requires a TornadoVM built from the cuTile branch, CUDA Toolkit 13.3+ and
 * driver R580+ (see env/versions.env, section "CUDA Tile").
 *
 * Usage:
 *   tornado --classpath . MatMulLadderFP16Tile [n] [executions]
 */
...
public class MatMulLadderFP16Tile {
    ...
    /** Tile shape for the CUDA Tile rung. A tile shape is part of the kernel's type. */
    private static final int TILE_BLOCK = 32;

    /**
     * The same GEMM with tiles as the unit of work.
     *
     * <p>
     * Read this against {@link #kcMma}: no lane, no warp, no fragment packing, no shared
     * memory, no {@code MMAShape}. The tile compiler chooses the tensor-core instruction, the
     * operand layouts and the thread count, and the cubin still disassembles to
     * {@code HMMA.16816.F32}.
     * </p>
     */
    public static void tiles(TileContext tc, HalfFloatArray a, HalfFloatArray b, FloatArray c, int n) {
        PartitionView aView = tc.partition(tc.view(a, n, n), TILE_BLOCK, TILE_BLOCK);
        PartitionView bView = tc.partition(tc.view(b, n, n), TILE_BLOCK, TILE_BLOCK);
        PartitionView cView = tc.partition(tc.view(c, n, n), TILE_BLOCK, TILE_BLOCK);

        int rowBlock = tc.bidX();
        int columnBlock = tc.bidY();

        Tile acc = tc.zeros(DType.F32, TILE_BLOCK, TILE_BLOCK);
    ...
    public static void main(String[] args) throws TornadoExecutionPlanException {
        ...
        System.out.printf("FP16 matrix-multiply ladder with a CUDA Tile rung: C = A * B, %dx%d%n", n, n);
        ...
        FloatArray cTile = new FloatArray(n * n);

        String[] labels = { "1. naive @Parallel", "2. KernelContext tiled", "3. KernelContext MMA (mma.sync)", //
                "4. TileContext (CUDA Tile, ct::mma)", "5. CUTLASS hgemm", "6. cuBLAS GemmEx FP16", //
                "7. cuBLAS GemmEx FP16->FP32" };
        ...
        times[0] = timeRung(new TaskGraph("naive") //
                ...
                .task("t", MatMulLadderFP16Tile::naive, a, b, cNaive, n) //
        ...
        times[1] = timeRung(new TaskGraph("tiled") //
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, a, b) //
                .task("t", MatMulLadderFP16Tile::kcTiled, new KernelContext(), a, b, cTiled, n) //
        ...
        times[2] = timeRung(new TaskGraph("mma") //
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, a, b) //
                .task("t", MatMulLadderFP16Tile::kcMma, new KernelContext(), a, b, cMma, n) //
        ...
        // rung 4 — CUDA Tile: the worker grid counts TILE BLOCKS, and the runtime pins the
        // thread block to 1x1x1 because that is what a tile launch requires.
        System.out.println(labels[3]);
        WorkerGrid2D wTile = new WorkerGrid2D(n / TILE_BLOCK, n / TILE_BLOCK);
        times[3] = timeRung(new TaskGraph("tile") //
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, a, b) //
                .task("t", MatMulLadderFP16Tile::tiles, new TileContext(), a, b, cTile, n) //
                .transferToHost(DataTransferMode.EVERY_EXECUTION, cTile), //
                new GridScheduler("tile.t", wTile), executions);
        ok &= validate("TileContext (CUDA Tile)", expected, toFloats(cTile), n);

        // rung 5 — CUTLASS, row-major
        System.out.println(labels[4]);
        times[4] = timeRung(new TaskGraph("cutlass") //
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, a, b) //
        ...
        // rungs 6 and 7 — cuBLAS is column-major, so swap the operands
        System.out.println(labels[5]);
        times[5] = timeRung(new TaskGraph("gemmex") //
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, a, b) //
        ...
        times[6] = timeRung(new TaskGraph("gemmexf32") //
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, a, b) //
        ...
        System.out.printf("scripts/compare-ladder.sh 22 collects both.%n");
