...
| Nsight Compute (`ncu`) hardware counters are usable on this machine. | Observed — **this supersedes the earlier "blocked" record.** | Two independent causes, both resolved: the wrong `ncu` version was first on `PATH`, and `RmProfilingAdminOnly=1`. Counters used throughout `results/raw/22-ncu-alignment-counters/` and `results/raw/31-load-batching-reorder/`. Use `/opt/nvidia/nsight-compute/2024.3.2/ncu`; the 2026.2.1 build on this box fails with "failed to connect to the CUDA driver". Historical failure record kept at `results/failures/08-nsight-compute-permission.md`. |
| TornadoVM's CUDA backend emitted shared-memory tile staging as one global load per shared store, serialising the loads. | Observed | `results/raw/31-load-batching-reorder/sass-schedules.txt` (`LSLSLSLS...`); `tornado --printKernel`; `long_scoreboard` 12.10 vs 1.45, issue rate 20.8% vs 50.9% in `ncu-counters.csv`. Fixed upstream by PR [#1079](https://github.com/beehive-lab/TornadoVM/pull/1079). |
| Batching those loads makes the register-tiled sgemm 1.650x faster, closing the gap to hand-written CUDA from 1.674x to 1.4%. | Observed, this-GPU/this-kernel only | `results/raw/31-load-batching-reorder/per-launch-durations.csv` (100 launches per arm, one build, flag toggled at run time); `kernel-summary.csv`. Do **not** restate as a general TornadoVM speedup — a kernel that does not stage through shared memory is unaffected. |
| The cause is the *instruction*, not the schedule: TornadoVM emits a generic `LD` (may target shared memory, so ptxas may not reorder it against a shared store) where hand-written CUDA emits `LDG` (proven global). | Observed | `results/raw/31-load-batching-reorder/sass-schedules.txt`; `cuobjdump -sass` of both cubins. Explains why adding `const`/`__restrict__` measured 1.00x. |
| The interleaved staging schedule cannot be expressed in CUDA C — three source orders compile to byte-identical SASS. | Observed (negative result, kept deliberately) | `results/raw/31-load-batching-reorder/ReorderProbe.cu`; 448 instructions and `LDG`x8 then `STS`x8 in all three variants. |
