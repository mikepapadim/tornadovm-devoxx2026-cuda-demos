...
# Extra shared-library directory a profile needs at run time. TornadoVM 7.0.0 ships
# lib/libtornado-cutlass.so linked against CUDA 13 (libcudart.so.13); on a box whose
# toolkit is 12.x, demos that use a CUTLASS task (12, 17, 18) abort without it.
TORNADO_LD_LIBRARY_PATH="$(eval echo "${TORNADO_LD_LIBRARY_PATH:-}")"
if [ -n "$TORNADO_LD_LIBRARY_PATH" ]; then
  if [ -d "$TORNADO_LD_LIBRARY_PATH" ]; then
    export LD_LIBRARY_PATH="$TORNADO_LD_LIBRARY_PATH${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
  else
    echo "[WARN] profile $TORNADO_SDK_PROFILE wants TORNADO_LD_LIBRARY_PATH=$TORNADO_LD_LIBRARY_PATH," >&2
    echo "       which does not exist. Demos needing it (CUTLASS: 12, 17, 18) will fail." >&2
  fi
fi
...
[ -n "$TORNADO_LD_LIBRARY_PATH" ] && echo "extra LD path       = $TORNADO_LD_LIBRARY_PATH"
